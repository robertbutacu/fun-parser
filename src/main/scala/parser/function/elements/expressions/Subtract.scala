...
case class Subtract[A: Fractional](expr1: Expression[A], expr2: Expression[A]) extends Expression[A] {
  override def compute(implicit n: Fractional[A]): Either[String, A] = computeBy(expr1, expr2, n.minus)
